...
const CourseCard = ({ course, instructor, type, role }) => {
  ...
  const getCourseDuration = () => {
    ...
    if (course.chapters && course.chapters.length > 0) {
      return course.chapters.reduce((sum, chapter) => {
        return sum + (chapter.video?.duration || 0);
      }, 0);
    }
  ...
  const totalDurationInSeconds = getCourseDuration();
  ...
  const handleStatusChange = async (newStatus) => {
    try {
      ...
      if (data.success) {
        ...
        createAlert(
          "success",
          data.message || "Course status updated successfully!"
        );
            ...
            <div className="text-lg font-semibold text-primaryColor mb-4">
              ...
              <span
                className={`ml-6 text-base font-semibold ${
                  offerType ? "text-green-500" : "text-secondaryColor3"
                }`}
              >
                {offerType}
              </span>
                      ...
                      <Image
                        ...
                        src={
                          instructor?.profile
                            ...
                            ? `${process.env.NEXT_PUBLIC_BACKEND_URL}${course.instructor.profile}`
