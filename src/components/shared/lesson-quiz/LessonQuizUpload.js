  ...
  return (
    <div className="bg-whiteColor dark:bg-whiteColor-dark p-6 rounded-md shadow-accordion dark:shadow-accordion-dark mt-6">
      <h2 className="text-xl font-bold mb-4 text-contentColor dark:text-contentColor-dark">
        <TranslatedText>Créer des questions de quiz</TranslatedText>
      </h2>
      ...
      {quizzes.map((quiz, qIndex) => (
        ...
        >
          ...
          >
            ➕ <TranslatedText>Ajouter une question</TranslatedText>{" "}
            {qIndex + 1}
          </button>
          ...
          {activeIndex === qIndex && (
            <div className="mt-2">
              ...
              <label className="block mb-1 font-medium text-sm text-contentColor">
                <TranslatedText>Réponse correcte</TranslatedText>
              </label>
              <select
      ...
      {quizzes.length > 0 && (
        ...
        >
          {loading ? (
            ...
            <TranslatedText>Soumettre toutes les questions</TranslatedText>
          )}
        </button>
      )}

      {message && (
        <p className="mt-4 text-sm text-contentColor text-center">{message}</p>
      )}
    </div>
  );
}
