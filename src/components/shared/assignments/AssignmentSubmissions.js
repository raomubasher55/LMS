      ...
      if (response.data.success) {
        ...
        setSubmissions(prev => prev.map(sub => 
          sub._id === submissionId 
            ? { ...sub, grade: { points: parseInt(gradeData.points), feedback: gradeData.feedback } }
            : sub
        ));
        
        // Clear grading data
        setGradingData(prev => ({ ...prev, [submissionId]: { points: '', feedback: '' } }));
          ...
          <div className="space-y-4">
            {submissions.map((submission) => (
              <div key={submission._id} className="border border-gray-200 rounded-lg p-4">
                      ...
                      {submission.files.map((file, index) => (
                        ...
                        >
                          ...
                          <span className="ml-2 text-xs text-gray-500">
                            ({file.size && !isNaN(file.size) ? (file.size / 1024).toFixed(1) : '0'} KB)
                          </span>
                        </a>
                      ))}
                    </div>
                ...
                {submission.grade && submission.grade.points !== undefined && submission.grade.points !== null ? (
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => handleGradingInputChange(submission._id, 'points', e.target.value)}
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) => handleGradingInputChange(submission._id, 'feedback', e.target.value)}
                          className="w-full p-2 border border-gray-300 rounded text-blackColor dark:text-blackColor-dark"
                          rows="2"
                        />
                      </div>
                    ...
                    <div className="mt-3">
                      ...
                      <select
                        ...
                        onChange={(e) => handleGradingInputChange(submission._id, 'status', e.target.value)}
                        className="w-full p-2 border border-gray-300 rounded text-blackColor dark:text-blackColor-dark"
                      >
                        <option value="graded">Graded (Final)</option>
                        <option value="resubmit">Require Resubmission</option>
                      </select>
                      <p className="text-xs text-gray-500 mt-1">
                        Select "Require Resubmission" to allow the student to submit again
                      </p>
                    </div>
                    <button
