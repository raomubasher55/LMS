...
const VideoPlayer = ({ videoData, title, poster, courseId, chapterId, onVideoCompleted }) => {
  ...
  return (
    <div className="relative w-full bg-black">
      ...
      {videoData && (videoData.vimeoId || videoData.embedCode || videoData.url) && !videoError ? (
        <div className="w-full h-auto min-h-[300px] lg:min-h-[500px]">
          {/* Try multiple rendering methods */}
          {(() => {
            // Method 1: Use the original embed code HTML if available
            if (videoData.embedCode && videoData.embedCode.includes('iframe')) {
              console.log('Using embedCode:', videoData.embedCode);
              return (
                <div 
                  className="w-full"
            ...
            // Method 2: Use the full URL HTML if available
            if (videoData.url && videoData.url.includes('iframe')) {
              console.log('Using url HTML:', videoData.url);
              return (
                <div 
                  className="w-full"
        ...
        <div className="w-full h-96 lg:h-[500px] flex items-center justify-center bg-gray-800">
          <div className="text-center text-white">
            ...
            <p className="text-sm text-gray-400">
              {videoError || (videoData ? 'Video embed code not found' : 'No video data provided')}
            </p>
            {videoData?.vimeoId && (
              <p className="text-xs text-gray-500 mt-2">Vimeo ID: {videoData.vimeoId}</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="absolute bottom-4 right-4 flex space-x-2">
          <div className="bg-black/70 text-white px-3 py-2 rounded-lg text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span>
                Duration: {formatTime(videoData.duration || 0)}
              </span>
            </div>
          </div>
