...
const StudentChatApp = () => {
  ...
  const fetchUserChats = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('StudentChatApp: Individual chat unread counts:', data.data?.map(chat => ({
          id: chat._id,
          participant: chat.participants?.find(p => p._id !== getCurrentUserId())?.firstName,
        ...
        if (data.data?.length > 0) {
          console.log('StudentChatApp: Setting active chat to first chat:', data.data[0]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.data.chatId || data.data._id) {
          ...
          console.error('StudentChatApp: New chat does not have chatId:', data.data);
        }
      } else {
        const errorData = await response.json();
        console.error('Failed to start instructor chat, status:', response.status);
        ...
        if (response.status === 403) {
          if (errorData.message?.includes('not enrolled')) {
            ...
            setError(errorData.message || 'Failed to start chat with instructor');
          }
        } else {
  ...
  const handleSendMessage = async (messageText) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('StudentChatApp: Message sent successfully, backend response:', data);
        ...
        <div className="xl:col-span-5">
          <div className="p-10px bg-whiteColor dark:bg-whiteColor-dark rounded-lg2">
            ...
            <div className="mb-4 space-y-2">
              ...
              >
                {showInstructorList ? 'Hide' : 'Message'} Instructors
              </button>
            </div>
            ...
            {showInstructorList && (
              <div className="mb-4 p-4 bg-gray-50 dark:bg-gray-800 rounded-md max-h-60 overflow-y-auto">
                <h4 className="font-medium mb-2 text-blackColor dark:text-blackColor-dark">Select Instructor:</h4>
                {instructorContacts.length === 0 ? (
                  <p className="text-gray-500 dark:text-gray-400">No instructors available. Enroll in courses to message instructors.</p>
                ) : (
                      ...
                      <div className="mt-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {instructor.courses.map((course) => (
                            <button
                              key={course._id}
                              onClick={() => startChatWithInstructor(instructor._id, course._id)}
            ...
            <div className="space-y-2">
              <h4 className="font-medium text-blackColor dark:text-blackColor-dark">Recent Chats:</h4>
              {chats.length === 0 ? (
                        ...
                        if (chat && (chat.chatId || chat._id)) {
                          ...
                          setChats(prev => prev.map(c => {
                            if ((c.chatId || c._id) === (chat.chatId || chat._id)) {
                              console.log('StudentChatApp: Resetting unread count for chat:', {
                                chatId: c._id,
                          ...
                          console.error('StudentChatApp: Invalid chat object', chat);
                        }
                      }}
                    >
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gray-300 rounded-full flex-shrink-0 flex items-center justify-center">
                          {otherParticipant?.firstName?.charAt(0) || otherParticipant?.name?.charAt(0) || '?'}
                        </div>
