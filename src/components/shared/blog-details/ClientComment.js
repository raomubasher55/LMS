  ...
  return (
    <div className="pt-50px pb-15px border-y border-borderColor2 dark:border-borderColor2-dark">
      <h4
        ...
        data-aos="fade-up"
      >
        ({comments?.length}) <TranslatedText>Commentaire</TranslatedText>
            ...
            <div className="flex-shrink-0">
              <Image
                src={comment.userId?.profile 
                  ? `${process.env.NEXT_PUBLIC_BACKEND_URL}${comment.userId.profile}` 
                  : defaultUserImage}
                alt="User"
                width={50}
                height={50}
                ...
                <ul>
                  {comment?.replies.map((reply, idx) => (
                    <li key={idx} className="flex gap-30px mb-10 lg:pl-100px">
                      <div className="flex-shrink-0">
                        <Image
                          src={reply.userId?.profile 
                            ? `${process.env.NEXT_PUBLIC_BACKEND_URL}${reply.userId.profile}` 
                            : defaultUserImage}
                          alt="User"
                          width={50}
                          height={50}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white dark:bg-blackColor-dark p-8 rounded-lg shadow-lg w-96 relative">
            ...
            <h2 className="text-xl font-bold mb-4"><TranslatedText>Répondre au commentaire</TranslatedText></h2>
            <textarea
              value={replyText}
            ...
            >
              <TranslatedText>Envoyer la réponse</TranslatedText>
            </button>
          </div>
        </div>
      )}
