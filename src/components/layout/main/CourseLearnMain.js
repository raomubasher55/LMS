          ...
          <div className="lg:col-span-2">
            ...
            {currentVideo ? (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden mb-6">
                <VideoPlayer
                  videoUrl={currentVideo.url ? `${process.env.NEXT_PUBLIC_BACKEND_URL}${encodeURI(currentVideo.url)}` : ''}
                  title={currentVideo.title}
                  poster={course.bannerImage ? `${process.env.NEXT_PUBLIC_BACKEND_URL}${course.bannerImage}` : null}
                  courseId={courseId}
                  chapterId={currentVideo.chapterId || currentVideo._id?.replace('_video', '')}
                ...
                <div className="p-4">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-500">
                    Duration: {Math.floor((currentVideo.duration || 0) / 60)} minutes
                  </p>
                </div>
              </div>
            ) : (
                ...
                {activeTab === 'quiz' && (
                  currentVideo ? (
                    <ChapterQuiz
                      ...
                      chapterId={currentVideo.chapterId || currentVideo._id?.replace('_video', '')}
                      quizzes={(() => {
                        const chapterId = currentVideo.chapterId || currentVideo._id?.replace('_video', '');
                    ...
                    <div className="quiz-no-video bg-gray-50 p-6 rounded-lg text-center">
                      <p className="text-gray-600">Please select a video to view the associated quiz.</p>
                    </div>
                  )
                )}
