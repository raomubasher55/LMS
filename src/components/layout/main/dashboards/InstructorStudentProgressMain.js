...
const InstructorStudentProgressMain = () => {
  ...
  const handleResetProgress = async (studentId, courseId, chapterId = null) => {
    ...
    try {
      ...
      createAlert('success', chapterId ? 'Chapter progress reset successfully' : 'Course progress reset successfully');
    ...
    return (
      <div className="instructor-student-progress">
        <div className="progress__header mb-6">
          ...
          <p className="text-gray-600">Track individual student progress and quiz performance</p>
        </div>
      ...
      <div className="progress__header mb-6">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4">
          ...
          <select
            ...
            onChange={(e) => setFilters(prev => ({ ...prev, restrictionStatus: e.target.value }))}
            className="border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-primaryColor focus:border-transparent"
          >
            <option value="all">All Students</option>
            <option value="restricted">Restricted Students</option>
            <option value="unrestricted">Unrestricted Students</option>
          </select>
            ...
            >
              ...
              <div className="student-courses p-6">
                {student.courses.map((course) => {
                  const statusColor = getRestrictionStatusColor(course.restrictionStatus);
                  const statusText = getRestrictionStatusText(course.restrictionStatus);

                  return (
                    <div key={course.courseId} className="course-progress mb-6 last:mb-0">
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h4 className="font-medium text-gray-800">{course.courseName}</h4>
                          <div className="flex items-center gap-4 mt-2">
                            <span className="text-sm text-gray-600">
                              Overall Progress: {course.overallProgress}%
                            </span>
                            <span className={`text-sm px-2 py-1 rounded-full bg-${statusColor}-100 text-${statusColor}-700`}>
                              {statusText}
                            </span>
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleResetProgress(student.studentId, course.courseId)}
                            disabled={resetting[`${student.studentId}-${course.courseId}-all`]}
                            className="text-sm bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 transition-colors disabled:opacity-50"
                          >
                            {resetting[`${student.studentId}-${course.courseId}-all`] ? 'Resetting...' : 'Reset Course'}
                          </button>
                        </div>
                      </div>
                          ...
                          <table className="w-full text-sm">
                            ...
                            <tbody>
                              {course.quizProgress.map((quiz) => (
                                <tr key={quiz.chapterId} className="border-t border-gray-200">
                                  <td className="px-4 py-2 font-medium">{quiz.chapterId}</td>
                                  <td className="px-4 py-2">
                                    {quiz.attemptsCount || quiz.attempts?.length || quiz.totalAttempts || 0}
                                    {quiz.totalAttempts > 0 && (!quiz.attempts || quiz.attempts.length === 0) && (
                                  ...
                                  <td className="px-4 py-2">
                                    <span className={`font-medium ${
                                      quiz.bestScore >= 60 ? 'text-green-600' : 'text-red-600'
                                    }`}>
                                      {quiz.bestScore}%
                                    </span>
                                  </td>
                                  <td className="px-4 py-2">
                                    <div className="flex flex-col gap-1">
                                      ...
                                      {quiz.restrictionStatus.timeRestricted && (
                                        <span className="text-orange-600 text-xs">Time Restricted</span>
                                      )}
                                      {quiz.restrictionStatus.videoReWatchRequired && (
                                        <span className="text-blue-600 text-xs">Video Rewatch</span>
                                      )}
                                      {quiz.restrictionStatus.instructorApprovalRequired && (
                                        <span className="text-red-600 text-xs">Needs Approval</span>
                                      )}
                                    </div>
                                  </td>
                                  ...
                                  <td className="px-4 py-2">
                                    <button
                                      onClick={() => handleResetProgress(student.studentId, course.courseId, quiz.chapterId)}
                                      disabled={resetting[`${student.studentId}-${course.courseId}-${quiz.chapterId}`]}
                                      className="text-xs bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 transition-colors disabled:opacity-50"
                                    >
                                      {resetting[`${student.studentId}-${course.courseId}-${quiz.chapterId}`] ? 'Resetting...' : 'Reset'}
                                    </button>
                                  </td>
                                </tr>
                              ))}
