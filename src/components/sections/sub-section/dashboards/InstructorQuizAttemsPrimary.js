...
const InstructorQuizAttemsPrimary = ({ title, path }) => {
  ...
  const applyFilters = () => {
    ...
    // Filter by course
    if (filters.courseId) {
      filtered = filtered.filter(attempt => attempt.courseId === filters.courseId);
    }
      ...
      switch (filters.status) {
        ...
        case 'restricted':
          filtered = filtered.filter(attempt => 
            attempt.restrictionStatus?.timeRestricted
          );
          break;
      }
    }
    ...
    return (
      <div className="instructor-quiz-attempts">
        <div className="attempts__header mb-6">
          ...
          <p className="text-gray-600">Monitor all quiz attempts across your courses</p>
        </div>
    ...
    <div className="instructor-quiz-attempts">
      ...
      {filteredAttempts.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <h3 className="text-lg font-medium text-gray-800 mb-2">No Quiz Attempts Found</h3>
          <p className="text-gray-600">No quiz attempts match the current filters.</p>
        </div>
      ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredAttempts.map((attempt) => (
                  <tr key={attempt.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{attempt.courseName}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{attempt.chapterId}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`text-sm font-medium ${
                        attempt.score >= 60 ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {attempt.score}%
                      </div>
                      {attempt.bestScore > 0 && attempt.bestScore !== attempt.score && (
