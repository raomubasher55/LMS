  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch announcements:', response.status, errorText);
      }
    } catch (error) {
      ...
      {announcements.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-contentColor dark:text-contentColor-dark mb-2">
            No announcements yet. Your instructors will post course updates here.
          </p>
          <p className="text-sm text-gray-500">
            To see announcements, you need to be enrolled in courses where instructors have posted announcements.
          </p>
          <div className="mt-4 text-xs text-gray-400">
            <p>Debug info: Component loaded successfully</p>
          ...
          {announcements.slice(0, 5).map((announcement) => (
            <div
              ...
              className={`border rounded-lg p-4 transition-all duration-200 cursor-pointer hover:shadow-md ${
                !announcement.isRead ? 'border-l-4 border-l-primaryColor bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-700'
              }`}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h4 className={`font-semibold ${!announcement.isRead ? 'text-primaryColor' : 'text-blackColor dark:text-blackColor-dark'}`}>
                      {announcement.title}
                      {!announcement.isRead && (
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400 mb-2">
                    <span className="font-medium">{announcement.courseId?.title}</span>
                    <span>•</span>
                    <span>{announcement.instructorId?.firstName} {announcement.instructorId?.lastName}</span>
                    <span>•</span>
                ...
                <div className="ml-4 flex-shrink-0">
                  <svg
                    className={`w-5 h-5 text-gray-400 transition-transform duration-200 ${
                      expandedAnnouncement === announcement._id ? 'transform rotate-180' : ''
                    }`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
