  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({ title: '', content: '', courseId: '', priority: 'medium' });
        fetchAnnouncements();
      } else {
        ...
        <table className="w-full text-left">
          <thead className="text-sm md:text-base text-blackColor dark:text-blackColor-dark bg-lightGrey5 dark:bg-whiteColor-dark leading-1.8 md:leading-1.8">
            <tr>
              ...
              <th className="px-5px py-10px md:px-5">Announcement</th>
              <th className="px-5px py-10px md:px-5">Priority</th>
          ...
          <tbody className="text-size-13 md:text-base text-contentColor dark:text-contentColor-dark font-normal">
            {announcements.length === 0 ? (
              <tr>
                <td colSpan="4" className="text-center py-8 text-contentColor dark:text-contentColor-dark">
                  No announcements found. Create your first announcement!
                </td>
              </tr>
            ) : (
              announcements.map((announcement, index) => (
                <tr key={announcement._id} className={`leading-1.8 md:leading-1.8 ${index % 2 === 0 ? '' : 'bg-lightGrey5 dark:bg-whiteColor-dark'}`}>
                  ...
                  <td className="px-5px py-10px md:px-5 font-normal text-wrap">
                    ...
                    <p className="text-sm mt-1">{announcement.content.substring(0, 100)}...</p>
                  </td>
