...
const WithdrawalForm = ({ availableBalance, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/withdrawals/request`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setErrors({ submit: errorData.message || 'Failed to submit withdrawal request' });
      }
    } catch (error) {
