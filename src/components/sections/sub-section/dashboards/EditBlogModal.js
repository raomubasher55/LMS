...
const EditBlogModal = ({ blog, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (blog) {
      setFormData({
        ...
        isCommentEnabled:
          blog.isCommentEnabled !== undefined ? blog.isCommentEnabled : true,
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/blogs/${blog._id}`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <div className="fixed inset-0 bg-[#000000ce] bg-opacity-50 flex items-center justify-center z-50 p-4">
