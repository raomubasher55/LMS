...
const Blogs = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/blogs?limit=3&sortBy=createdAt&sortOrder=desc`);
      ...
      <div className="container py-100px">
        ...
        {blogs.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-30px">
            {blogs.map((blog, idx) => {
              console.log(blog.author?.profile)
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <div className="w-11 h-11">
                          <Image
                            src={`${process.env.NEXT_PUBLIC_API_URL}${blog.author?.profile}`}
