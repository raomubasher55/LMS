    ...
    const fetchCourses = async () => {
      try {
        ...
        setFilterInputs([
          ...
          {
            name: "Tag",
            inputs: Array.from(tagSet),
            scrollable: tagSet.size > 10
          },
          // {
          //   name: "Skill Level",
          //   inputs: [
...
const getAllFilteredCourses = (filterableCourses, filterObject) => {
  ...
  return filterableCourses?.filter(
    ({ category, tags, skillLevel }) =>
      (!currentCategories?.length || currentCategories.includes(category)) &&
      (!currentTags?.length || 
        (Array.isArray(tags) && currentTags.some(tag => tags.includes(tag))) &&
  ...
  return (
    <div>
      ...
      >
        {category && (
          <h1 className="text-2xl font-bold mb-6 dark:text-whiteColor text-blackColor">
          ...
          {isNotSidebar ? (
            ...
            <div className="md:col-start-1 md:col-span-4 lg:col-span-3">
              <div className="flex flex-col ">
                ...
                {/* categories  */}
                {filterInputs?.map(({ name, inputs , scrollable  }, idx) => (
                  <div
                    key={idx}
                    className="pt-30px pr-15px pl-10px pb-23px 2xl:pt-10 2xl:pr-25px 2xl:pl-5 2xl:pb-33px mb-30px border border-borderColor dark:border-borderColor-dark"
                    data-aos="fade-up"
                    ...
                    <ul
                      className={`flex flex-col ${
                        name === "Categories"
                          ...
                          ? "gap-y-23px"
                          : "gap-y-10px"
                      } ${scrollable ? 'max-h-[300px] overflow-y-auto scrollable-tags' : ''}`}
                    >
                      {name === "Categories"
